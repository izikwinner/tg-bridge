...
describe('ProgressTracker', () => {
  ...
  test('Task tool via onTool → marks dispatch done (no tool entry)', () => {
    const t = new ProgressTracker({ now: () => 1000, startMs: 1000 })
    t.onTaskStart({ subagent_type: 'researcher', description: 'find logs' })
    t.onTool('Task', { subagent_type: 'researcher', description: 'find logs' })
    const out = t.render()
    expect(out).toContain('agents:')
    expect(out).toContain('x researcher')
    expect(out).not.toContain('[A] task')
  })

  test('subagent: running then done updates marker', () => {
    const t = new ProgressTracker({ now: () => 1000, startMs: 1000 })
    t.onTaskStart({ subagent_type: 'reviewer', description: 'review PR' })
    let out = t.render()
    expect(out).toContain('&gt; reviewer')
    t.onTaskEnd({ subagent_type: 'reviewer' })
    out = t.render()
    expect(out).toContain('x reviewer')
  })

  test('multiple subagents show progress bar', () => {
    const t = new ProgressTracker({ now: () => 1000, startMs: 1000 })
    t.onTaskStart({ subagent_type: 'a' }); t.onTaskEnd({ subagent_type: 'a' })
    t.onTaskStart({ subagent_type: 'b' }); t.onTaskEnd({ subagent_type: 'b' })
    t.onTaskStart({ subagent_type: 'c' })
    const out = t.render()
    expect(out).toContain('x a')
    expect(out).toContain('x b')
    expect(out).toContain('&gt; c')
